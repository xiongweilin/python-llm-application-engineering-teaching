...
def main() -> int:
    ...
    if shared_css_path.is_file():
        ...
        for marker in ("--course-accent", "--course-card-alpha: 0", "--course-control-alpha: 0", "--course-text-alpha: 0.7", "--course-video-brightness: 0.9", "--course-video-veil-alpha: 0.1", "rgb(255 253 248 / var(--course-card-alpha))", "rgb(24 36 52 / var(--course-card-alpha))", "rgb(25 37 54 / var(--course-text-alpha))", "rgb(255 255 255 / var(--course-control-alpha))", "rgb(243 239 230 / var(--course-video-veil-alpha))", "filter: brightness(var(--course-video-brightness))", ".course-nav", ".course-context-bar", ".course-background-video", ".course-background-overlay", "opacity: 0.7", "prefers-reduced-motion: reduce", ".question-button", ".page-status", ".session-terms", ".term-grid", ".session-controls", ".prototype-switcher", ".completion", "textarea", "@media"):
