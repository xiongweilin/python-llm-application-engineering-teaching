...
def main() -> int:
    ...
    required_text = {
        ...
        ROOT / "reference" / "0002-course-progress.html": ("四个阶段", "十七个学习环", "约 90–125", "环 0", "环 16", "会话 2“重试、防重复与批处理”", "每题先看示范和执行轨迹", "翻译层", "当前阶段相称的条件变化", "会话 3“模块、诊断与授权边界”", "数学语言、概率、统计与证据", "线性代数与张量", "微积分、决策与约束优化", "状态演化、随机过程与序贯决策", "博弈、信息与有限机制分析", "语言模型核心计算", "Transformer 机制", "训练与数值核心", "按需扩展", "不要求实际完成系统微调", "嵌入、检索与 RAG", "Agent、运行决策与人机合作", "实验、因果评估与系统可靠性", "背门路径", "碰撞点（collider）", "最终能力契约", "离线有限模型、仿真或回放", "最终综合项目与论文阅读", "十一项最终能力如何落到路线", "最终能力—关口覆盖索引"),
        ...
        ROOT / "NOTES.md": ("粒度层级固定", "三个不同问题", "A“专注隧道”", "纵向切片", "会话 3“模块、诊断与授权边界”", "共九项任务", "示范优先、局部补全和有界变化", "逐行旁注", "再激活型学习者", "editable locus", "现实含义 → 程序责任 → 代码位置 → Python 写法 → 学生操作 → 运行验证", "pass", "建议路径", "活动文档保持最小且充分"),
        ...
        ROOT / "SESSION-PAGE-CONTRACT.md": ("2026-08-18", "唯一最终能力事实源 → 阶段 → 学习环", "本会话中文术语", "综合复习会话", "候选参考页", "模块、诊断与授权边界", "四阶段、十七环", "教学坡度", "教师示范 → 共同追踪输入/输出 → 局部补全", "再激活型学习者支持", "现实含义 → 程序责任 → 代码位置 → Python 写法 → 学生操作 → 运行验证", "术语来源协议", "三种难度分离", "五类帮助", "editable locus", "第一次出现 `pass`", "可观察的对应关系/映射", "唯一表达式位置", "建议路径", "完整讲解", "逐行旁注", "不要求学习者从空白页自行设计"),
    ...
    interactive_contracts = {
        ...
        ROOT / "lessons" / "0012-failure-retry-stop.html": (
            ...
            '唯一可修改位置',
            'pass</code> 先解释一次',
            'retry-mapping-attempt-1',
            'retry-mapping-attempt-2',
            'retry-mapping-attempt-3',
            'retry-mapping-boundary',
            'check-retry-mapping',
